...
/**
 * resolve the settings in a map file to the final color scheme
 * for use/export
 */
function mapToScheme(map, palette) {
  return _.transform(map, function(result, value, key) {
    if(_.isObject(value)) {
      _.assign(result, colorsForCombo(map, palette, key, value));
      return true;  // continue
    }
    result[key] = colorForVar(map, palette, key, value);
  });
}
